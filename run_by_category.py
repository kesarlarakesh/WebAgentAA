...
from sheets_reader import get_tasks_by_category
...
async def main():
    ...
    try:
        # Get tasks filtered by category (reads sheet_name from .env)
        tasks = get_tasks_by_category(spreadsheet_id, category, active_only=True)
        
        if not tasks:
            print(f"❌ No active tasks found for category: '{category}'")
            return
        ...
        for i, task in enumerate(tasks, 1):
            print(f"{i}. [{task['priority']}] {task['scenario_name']}")
        ...
        # Print summary
        print_summary(results, title=f"Category '{category}' Tests Complete")
